...
const auth = () => {
     axios.post(api_url,
        {username: user_details.name, password: user_details.password},
...
<template>
    ...
    <div class="px-5 min-h-screen flex justify-content-center align-items-center">
        <div class="border-1 surface-border surface-card border-round py-7 px-4 md:px-7 z-1">
            <div class="mb-4">
                <div class="text-900 text-xl font-bold mb-2">Kirish</div>
                <span class="text-600 font-medium">Iltimos ma'lumotlaringizni kiriting</span>
            </div>
            <div class="flex flex-column">
                <span class="p-input-icon-left w-full mb-4">
                    <i class="pi pi-envelope"></i>
                    <InputText id="name" type="text" class="w-full md:w-25rem" placeholder="Isim"  v-model='user_details.name'/>
                </span>
                <span class="p-input-icon-left w-full mb-4">
                    <i class="pi pi-lock"></i>
                    <InputText id="password" type="password" class="w-full md:w-25rem" placeholder="Parol"  v-model='user_details.password'/>
                </span>
                <Button label="Kirish" class="w-full" @click="auth()"></Button>
            </div>
        </div>
    </div>
    <AppConfig simple />
